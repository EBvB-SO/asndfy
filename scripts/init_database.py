...
def init_exercises():
    ...
    try:
        ...
        if db.query(Exercise).count() > 0:
            logger.info("Exercises already initialized")
            return
        
        # Add this to backend/app/scripts/init_database.py in the init_exercises function
# Replace the incomplete exercise_data array with:
...
exercise_data = [
    ...
    },
    {
        ...
        for badge_data in badges:
            badge = Badge(
                category_id=categories[badge_data["category"]],
                name=badge_data["name"],
                description=badge_data["description"],
                icon_name=badge_data["icon_name"],
                how_to_earn=badge_data["how_to_earn"]
            )
            db.add(badge)
...
def main():
    """Initialize database with exercises and badges"""
...
if __name__ == "__main__":
    main(),
        ...
        db.commit()
        logger.info(f"Initialized {len(exercise_data)} exercises")
        
    except Exception as e:
        db.rollback()
        logger.error(f"Error initializing exercises: {str(e)}")
        raise
    finally:
        db.close()
...
def init_badges():
    ...
    try:
        ...
        if db.query(Badge).count() > 0:
            logger.info("Badges already initialized")
            return
        
        # Create badge categories
        categories = {
            "Climbing Styles": None,
            "Training Plans": None,
            "Projects": None,
            "Project Logs": None
        }
        ...
        for cat_name in categories:
            category = BadgeCategory(name=cat_name)
            db.add(category)
            db.flush()
